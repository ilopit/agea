...
audio_system::on_init(gs::state&)
{
    ...
    if (r != MA_SUCCESS)
    {
        ALOG_WARN("audio_system: no usable audio device (ma_result={}); audio disabled", (int)r);
        m_enabled = false;
        return;
    }
    m_enabled = true;
    ALOG_INFO("audio_system: initialized");
}

void
audio_system::load_clip(root::audio_clip& clip)
{
    if (!m_enabled)
    {
        return;
    ...
    if (data.size() == 0)
    {
        ALOG_WARN("audio_system: clip {} has no data", id.cstr());
        return;
    }
    ...
    {
        ALOG_ERROR("audio_system: failed to decode clip {}", id.cstr());
        return;
    }
    ...
    if (total == 0)
    {
        ALOG_ERROR("audio_system: clip {} decoded to zero frames", id.cstr());
        return;
    }
...
bool
audio_system::is_clip_loaded(const utils::id& clip_id) const
{
    return m_be->clips.find(clip_id) != m_be->clips.end();
}

void
audio_system::play(const utils::id& voice_id, const utils::id& clip_id, const play_params& params)
{
    if (!m_enabled)
    {
        return;
    ...
    {
        ALOG_WARN("audio_system: play of unloaded clip {}", clip_id.cstr());
        return;
    }
    ...
    {
        ALOG_ERROR("audio_system: failed to create audio buffer for {}", clip_id.cstr());
        return;
    }
    ...
    {
        ALOG_ERROR("audio_system: failed to create sound for {}", clip_id.cstr());
    ...
    ALOG_INFO("audio_system: playing voice {} clip {} (spatial={}, loop={}, vol={}, pos=({},{},{}))",
              voice_id.cstr(),
              clip_id.cstr(),
              params.spatial,
              params.looping,
...
void
audio_system::stop(const utils::id& voice_id)
{
    ...
    ALOG_INFO("audio_system: stopped voice {}", voice_id.cstr());
}
...
bool
audio_system::is_voice_playing(const utils::id& voice_id) const
{
    auto it = m_be->voices.find(voice_id);
    if (it == m_be->voices.end())
    {
...
void
audio_system::collect_active_voice_ids(std::vector<utils::id>& out) const
{
    out.reserve(out.size() + m_be->voices.size());
    for (const auto& [id, v] : m_be->voices)
    {
        out.push_back(id);
    }
}
...
void
audio_system::set_voice_position(const utils::id& voice_id, const glm::vec3& pos)
{
    auto it = m_be->voices.find(voice_id);
    if (it == m_be->voices.end())
    {
    ...
    ALOG_INFO("audio_system: voice {} moved to ({},{},{})", voice_id.cstr(), pos.x, pos.y, pos.z);
}

void
audio_system::set_voice_volume(const utils::id& voice_id, float volume)
{
    auto it = m_be->voices.find(voice_id);
    if (it == m_be->voices.end())
    {
...
void
audio_system::set_listener(const glm::vec3& pos, const glm::vec3& forward, const glm::vec3& up)
{
    if (!m_enabled)
    {
        return;
...
void
audio_system::set_master_volume(float v)
{
    if (!m_enabled)
    {
        return;
