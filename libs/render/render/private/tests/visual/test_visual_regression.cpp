...
protected:
    ...
    // Matches solid_color_alpha_material__gpu layout (generated by argen)
    // NOTE: field order matches argen output — opacity before shininess
    struct solid_color_alpha_mat_gpu
    {
        alignas(4) uint32_t texture_indices[KGPU_MAX_TEXTURE_SLOTS];
        alignas(4) uint32_t sampler_indices[KGPU_MAX_TEXTURE_SLOTS];
        alignas(16) glm::vec3 ambient;
    ...
    make_solid_color_alpha_gpu_data(const glm::vec3& ambient,
                                    ...
                                    float opacity)
    {
        solid_color_alpha_mat_gpu mat_gpu{};
        for (int i = 0; i < KGPU_MAX_TEXTURE_SLOTS; ++i)
        {
            mat_gpu.texture_indices[i] = UINT32_MAX;
            mat_gpu.sampler_indices[i] = UINT32_MAX;
