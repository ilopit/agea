...
lightmap_baker::bake(const bake::bake_settings& settings)
{
    ...
    vk_utils::descriptor_builder::begin(&layout_cache, &desc_alloc)
        ...
        .bind_image(
            6, 1, &ao_lightmap_info, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE, VK_SHADER_STAGE_COMPUTE_BIT)
        .build(ds_ao, dsl_ao);
    ...
    auto compile_bake_shader = [&](const char* source_vfs_path, const char* name) -> bake_shader
    {
        ...
        {
            ALOG_ERROR("lightmap_baker: failed to load source from {}", APATH(real.value()).str());
            return s;
        }
    ...
    device->immediate_submit(
        [&](VkCommandBuffer cmd)
        {
            ...
            vkCmdClearColorImage(
                cmd, img_gbuf_pos->image(), VK_IMAGE_LAYOUT_GENERAL, &zero_color, 1, &clear_range);
            vkCmdClearColorImage(cmd,
                                 img_gbuf_normal->image(),
                                 VK_IMAGE_LAYOUT_GENERAL,
                                 &zero_color,
                                 1,
                                 &clear_range);
            vkCmdClearColorImage(
                cmd, img_lightmap->image(), VK_IMAGE_LAYOUT_GENERAL, &zero_color, 1, &clear_range);
            vkCmdClearColorImage(cmd,
                                 img_lightmap_bounce->image(),
                                 VK_IMAGE_LAYOUT_GENERAL,
                                 &zero_color,
                                 1,
                                 &clear_range);
            vkCmdClearColorImage(
                cmd, img_ao->image(), VK_IMAGE_LAYOUT_GENERAL, &zero_color, 1, &clear_range);
            vkCmdClearColorImage(cmd,
