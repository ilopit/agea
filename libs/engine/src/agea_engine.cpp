...
vulkan_engine::init()
{
    ...
    m_editor = glob::game_editor::create();
    m_input_manager = glob::input_manager::create();
    m_resource_locator = glob::resource_locator::create();
    m_config = glob::config::create();
...
vulkan_engine::load_level(const utils::id& level_id)
{
    auto result = model::level_constructor::load_level_id(
        *glob::level::get(), glob::config::get()->level, glob::class_objects_cache_set_view::get(),
