...
// ============================================================================
// Central dispatch — switch on cmd_kind, run the matching process(), then
// destruct (the arena only rewinds; ~T() releases non-trivial members).
// ============================================================================
...
dispatch(render_command_base* cmd, render_exec_context& ctx)
{
    switch (cmd->cmd_kind)
    {
    case render_cmd_kind::update_transform:
        run_and_destroy<update_transform_cmd>(cmd, ctx);
        break;
    case render_cmd_kind::set_outline:
        run_and_destroy<set_outline_cmd>(cmd, ctx);
        break;
    case render_cmd_kind::create_mesh:
        run_and_destroy<create_mesh_cmd>(cmd, ctx);
        break;
    case render_cmd_kind::destroy_mesh:
        run_and_destroy<destroy_mesh_cmd>(cmd, ctx);
        break;
    case render_cmd_kind::create_texture:
        run_and_destroy<create_texture_cmd>(cmd, ctx);
        break;
    case render_cmd_kind::destroy_texture:
        run_and_destroy<destroy_texture_cmd>(cmd, ctx);
        break;
    case render_cmd_kind::create_shader_effect:
        run_and_destroy<create_shader_effect_cmd>(cmd, ctx);
        break;
    case render_cmd_kind::destroy_shader_effect:
        run_and_destroy<destroy_shader_effect_cmd>(cmd, ctx);
        break;
    case render_cmd_kind::create_material:
        run_and_destroy<create_material_cmd>(cmd, ctx);
        break;
    case render_cmd_kind::update_material:
        run_and_destroy<update_material_cmd>(cmd, ctx);
        break;
    case render_cmd_kind::destroy_material:
        run_and_destroy<destroy_material_cmd>(cmd, ctx);
        break;
    case render_cmd_kind::create_object:
        run_and_destroy<create_object_cmd>(cmd, ctx);
        break;
    case render_cmd_kind::update_object:
        run_and_destroy<update_object_cmd>(cmd, ctx);
        break;
    case render_cmd_kind::destroy_object:
        run_and_destroy<destroy_object_cmd>(cmd, ctx);
        break;
    case render_cmd_kind::create_chunk_mesh:
        run_and_destroy<create_chunk_mesh_cmd>(cmd, ctx);
        break;
    case render_cmd_kind::create_light:
        run_and_destroy<create_light_cmd>(cmd, ctx);
        break;
    case render_cmd_kind::update_light:
        run_and_destroy<update_light_cmd>(cmd, ctx);
        break;
    case render_cmd_kind::select_directional_light:
        run_and_destroy<select_directional_light_cmd>(cmd, ctx);
        break;
    case render_cmd_kind::destroy_light:
        run_and_destroy<destroy_light_cmd>(cmd, ctx);
        break;
    case render_cmd_kind::create_skinned_mesh:
        run_and_destroy<create_skinned_mesh_cmd>(cmd, ctx);
        break;
    case render_cmd_kind::create_lightmap:
        run_and_destroy<create_lightmap_cmd>(cmd, ctx);
        break;
    case render_cmd_kind::destroy_lightmap:
        run_and_destroy<destroy_lightmap_cmd>(cmd, ctx);
        break;
    case render_cmd_kind::apply_bones:
        run_and_destroy<apply_bones_cmd>(cmd, ctx);
        break;
    }
}
