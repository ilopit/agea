...
class TestParseAttributes(unittest.TestCase):
    ...
    def test_single_line_attributes(self):
        lines = ['KRG_ar_class(key1=value1, key2=value2)']
        index, properties = arapi.parser.parse_attributes("KRG_ar_class", lines, 0, 0)
    ...
    def test_multi_line_attributes(self):
        lines = [
            'KRG_ar_class(key1=value1,',
            'key2=value2,',
            'key3=value3)'
        ]
        index, properties = arapi.parser.parse_attributes("KRG_ar_class", lines, 0, 2)
...
class TestParseFile(unittest.TestCase):
    ...
    def test_parse_model_overrides(self):
        ...
        with open(test_file, 'w') as f:
            f.write('KRG_ar_model_overrides()\n')
    ...
    def test_parse_render_overrides(self):
        test_file = os.path.join(self.temp_dir, "test.h")
        with open(test_file, 'w') as f:
            f.write('KRG_ar_render_overrides()\n')
    ...
    def test_parse_class(self):
        ...
        self.assertEqual(self.context.types[0].kind, arapi.types.kryga_type_kind.CLASS)
    ...
    def test_parse_struct(self):
        ...
        self.assertEqual(self.context.types[0].kind, arapi.types.kryga_type_kind.STRUCT)
    ...
    def test_parse_property(self):
        ...
        with open(test_file, 'w') as f:
            f.write('KRG_ar_class()\n')
            f.write('class TestClass {\n')
            f.write('  KRG_ar_property("category=test", "access=all")\n')
            f.write('  int m_value;\n')
            f.write('};\n')
    ...
    def test_parse_property_with_default(self):
        ...
        with open(test_file, 'w') as f:
            f.write('KRG_ar_class()\n')
            f.write('class TestClass {\n')
            f.write('  KRG_ar_property("default=true")\n')
    ...
    def test_parse_property_invalidates(self):
        ...
        with open(test_file, 'w') as f:
            f.write('KRG_ar_class()\n')
            f.write('class TestClass {\n')
            f.write('  KRG_ar_property("invalidates=render", "invalidates=transform")\n')
            f.write('  int m_value;\n')
            f.write('};\n')
    ...
    def test_parse_property_check(self):
        ...
        with open(test_file, 'w') as f:
            f.write('KRG_ar_class()\n')
            f.write('class TestClass {\n')
            f.write('  KRG_ar_property("check=not_same")\n')
            f.write('  int m_value;\n')
            f.write('};\n')
    ...
    def test_parse_property_hint(self):
        ...
        with open(test_file, 'w') as f:
            f.write('KRG_ar_class()\n')
            f.write('class TestClass {\n')
            f.write('  KRG_ar_property("hint=hint1")\n')
            f.write('  int m_value;\n')
            f.write('};\n')
    ...
    def test_parse_external_type(self):
        test_file = os.path.join(self.temp_dir, "test.h")
        with open(test_file, 'w') as f:
            f.write('KRG_ar_external_type()\n')
            f.write('struct ::external::Type;\n')
        
        arapi.parser.parse_file(test_file, "include/test.h", "test_module", self.context)
        self.assertEqual(len(self.context.types), 1)
        self.assertEqual(self.context.types[0].kind, arapi.types.kryga_type_kind.EXTERNAL)
        self.assertIn("Type", self.context.types[0].name)

    def test_parse_package(self):
        test_file = os.path.join(self.temp_dir, "test.h")
        with open(test_file, 'w') as f:
            f.write('KRG_ar_package(model.has_types_overrides=true, render.has_overrides=false)\n')
    ...
    def test_parse_package_dependencies(self):
        test_file = os.path.join(self.temp_dir, "test.h")
        with open(test_file, 'w') as f:
            f.write('KRG_ar_package(dependancies=dep1:dep2:dep3)\n')
            f.write('// extra line to avoid index error\n')
    ...
    def test_parse_class_with_namespace(self):
        self.context.root_namespace = "kryga"
        test_file = os.path.join(self.temp_dir, "test.h")
        with open(test_file, 'w') as f:
            f.write('KRG_ar_class()\n')
            f.write('class TestClass {\n')
        ...
        arapi.parser.parse_file(test_file, "include/test.h", "test_module", self.context)
        self.assertIn("kryga::test_module::TestClass", self.context.types[0].full_name)
    ...
    def test_parse_multiple_properties(self):
        ...
        with open(test_file, 'w') as f:
            f.write('KRG_ar_class()\n')
            f.write('class TestClass {\n')
            f.write('  KRG_ar_property("category=test1")\n')
            f.write('  int m_value1;\n')
            f.write('  KRG_ar_property("category=test2")\n')
    ...
    def test_parse_property_all_keys(self):
        ...
        with open(test_file, 'w') as f:
            f.write('KRG_ar_class()\n')
            f.write('class TestClass {\n')
            f.write('  KRG_ar_property("category=cat", "serializable=true", "access=all", "gpu_data=gpu", "copyable=yes", "ref=false")\n')
            f.write('  int m_value;\n')
            f.write('};\n')
    ...
    def test_parse_property_updatable(self):
        ...
        with open(test_file, 'w') as f:
            f.write('KRG_ar_class()\n')
            f.write('class TestClass {\n')
            f.write('  KRG_ar_property("updatable=no")\n')
            f.write('  int m_value;\n')
            f.write('};\n')
    ...
    def test_parse_property_invalid_access(self):
        ...
        with open(test_file, 'w') as f:
            f.write('KRG_ar_class()\n')
            f.write('class TestClass {\n')
            f.write('  KRG_ar_property("access=invalid_access")\n')
            f.write('  int m_value;\n')
            f.write('};\n')
    ...
    def test_parse_property_invalid_serializable(self):
        ...
        with open(test_file, 'w') as f:
            f.write('KRG_ar_class()\n')
            f.write('class TestClass {\n')
            f.write('  KRG_ar_property("serializable=yes")\n')
            f.write('  int m_value;\n')
            f.write('};\n')
    ...
    def test_parse_property_invalid_copyable(self):
        ...
        with open(test_file, 'w') as f:
            f.write('KRG_ar_class()\n')
            f.write('class TestClass {\n')
            f.write('  KRG_ar_property("copyable=true")\n')
            f.write('  int m_value;\n')
            f.write('};\n')
