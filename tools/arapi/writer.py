...
def write_gpu_struct(type_obj: arapi.types.kryga_type,
  ...
  has_tex_slots = any(f.is_texture_slot for f in fields)
  tex_slot_include = "\n#include <core/reflection/reflection_type.h>" if has_tex_slots else ""
...
#include <cstring>
#include <cstddef>
#include <cstdint>{tex_slot_include}
#endif
...
""")
  ...
  # Generate inline texture slot collect function in the header
  tex_fields = [f for f in fields if f.is_texture_slot]
  if tex_fields:
    file_buffer.append(f"""
inline unsigned int collect_texture_slots__{struct_name}(
    const {full_model_type}& src,
    ::kryga::reflection::gpu_texture_slot_ref* out)
{{
    const auto* blob = reinterpret_cast<const uint8_t*>(&src);
""")
    for i, field in enumerate(tex_fields):
      file_buffer.append(f"    out[{i}] = {{ {field.texture_slot_index}, blob + offsetof({full_model_type}, {field.name}) }};\n")
    file_buffer.append(f"    return {len(tex_fields)};\n}}\n")

  file_buffer.append("#endif // !KRG_GPU_STRUCT_ONLY\n")
...
def _collect_texture_slot_fields(type_obj: arapi.types.kryga_type,
                                  context: arapi.types.file_context) -> List[GpuField]:
  fields = _collect_gpu_fields_from_type(type_obj, context)
  return [f for f in fields if f.is_texture_slot]
...
def generate_enforcer(should_generate: bool, name: str) -> str:
  if not should_generate:
    return EMPTY_STRING
  return f"""private:\\\n  static bool package_{name}_enforcer();\\\n  static inline bool has_{name}_types = package_{name}_enforcer();\\\npublic:\\\n"""
...
namespace kryga::reflection {{ struct gpu_texture_slot_ref; }}
...
"""
    ...
    tex_fields = _collect_texture_slot_fields(ar_type, context)
    if tex_fields:
      gpu_fwd_decl += f"unsigned int collect_texture_slots__{struct_name}(const {full_type}&, ::kryga::reflection::gpu_texture_slot_ref*);\n"
      gpu_friend += f"    friend unsigned int ::collect_texture_slots__{struct_name}(const {full_type}&, ::kryga::reflection::gpu_texture_slot_ref*); \\\n"
...
#define KRG_gen_meta__{context.module_name}_package_editor
...
#if defined(KRG_build__editor)
#undef  KRG_gen_meta__{context.module_name}_package_editor
#define KRG_gen_meta__{context.module_name}_package_editor \\
{generate_enforcer(context.editor_has_overrides, "editor")}{generate_builder(context.editor_has_overrides, "editor_types")} \\
private:
#endif
...
#define KRG_gen_meta__{context.module_name}_package \\
  ...
  KRG_gen_meta__{context.module_name}_package_editor \\
...
""")
  ...
  if prop.gpu_texture_slot >= 0:
    file_buffer.append(f"        p->gpu_texture_slot  = {prop.gpu_texture_slot};\n")
...
""")
      ...
      file_buffer.append(f"""        p->json_set = [](::kryga::reflection::property_context__json_set& ctx) -> ::kryga::result_code {{
            ...
            load_ctx.owner_obj = ctx.obj;
            load_ctx.obj = reinterpret_cast<::kryga::blob_ptr>(&tmp);
            load_ctx.jc = ctx.jc;
            if (ctx.p->rtype->json_load(load_ctx) != ::kryga::result_code::ok)
                return ::kryga::result_code::failed;
            static_cast<type&>(*ctx.obj).set_{prop.name_cut}(tmp);
            return ::kryga::result_code::ok;
        }};
""")
...
def write_glue_link_source(included_modules: List[str], output_dir: str) -> None:
  """Emit the glue translation unit that references the link anchor of every package
    that opted in via KRG_ar_package(included = true).

    libs/glue compiles the .cpp and the engine links it and calls link_all_packages()
    once. That call references each anchor, so the linker retains the packages' static
    registration TUs — without the package being referenced anywhere else, and without
    any MSVC-specific /WHOLEARCHIVE. Portable to clang/Android (plain symbol references).
    """
  glue_dir = os.path.join(output_dir, "packages", "glue")
  header_path = os.path.join(glue_dir, "public", "include", "glue", "glue_link.ar.h")
  source_path = os.path.join(glue_dir, "private", "src", "glue_link.ar.cpp")
  os.makedirs(os.path.dirname(header_path), exist_ok=True)
  os.makedirs(os.path.dirname(source_path), exist_ok=True)

  header = (
      "// Auto-generated by argen (--type bind). DO NOT EDIT.\n"
      "#pragma once\n"
      "namespace kryga::glue\n{\n"
      "// Forces the linker to retain the static registration of every package that\n"
      "// opted in via KRG_ar_package(included = true). Call once during engine init.\n"
      "int link_all_packages();\n"
      "}  // namespace kryga::glue\n")

  extern_decls = "".join(
      f"extern \"C\" int kryga_force_link_{m}();\n" for m in included_modules)
  sink_lines = "".join(
      f"    sink += kryga_force_link_{m}();\n" for m in included_modules)
  ...
  header_buf = arapi.utils.FileBuffer(header_path)
  header_buf.append(header)
  header_buf.write_if_changed()

  source_buf = arapi.utils.FileBuffer(source_path)
  source_buf.append(source)
  source_buf.write_if_changed()
...
// Link anchor: the generated glue library (libs/glue) references this symbol so the
// linker retains this translation unit — and the static schedules above — even though
// nothing else in the program references the package. extern "C" keeps the symbol name
// un-mangled so the glue can forward-declare it without the package's headers.
extern "C" int kryga_force_link_{fc.module_name}() {{ return 0; }}
...
""")
  ...
  # Generate static GPU texture collect wrapper functions
  for type_obj in fc.types:
    if type_obj.kind != arapi.types.kryga_type_kind.CLASS:
      continue
    tex_fields = _collect_texture_slot_fields(type_obj, fc)
    if not tex_fields:
      continue
    struct_name = f"{type_obj.name}__gpu"
    full_type = type_obj.get_full_type_name()
    file_buffer.append(f"""
static uint32_t gpu_texture_collect__{type_obj.name}(const void* src, ::kryga::reflection::gpu_texture_slot_ref* out)
{{
    return collect_texture_slots__{struct_name}(
        *static_cast<const {full_type}*>(src), out);
}}
""")
...
""")
        ...
        if has_texture_slots:
          file_buffer.append(f"  type_rt->gpu_texture_collect     = gpu_texture_collect__{type_obj.name};\n")
          file_buffer.append(f"  type_rt->gpu_texture_slot_count  = {len(tex_fields)};\n")
        file_buffer.append("}")
...
def write_editor_types_reflection(package_ar_file: str, fc: arapi.types.file_context) -> None:
  """Write editor types reflection implementation file.

    Generates package.{name}.editor.ar.cpp that registers json_save/json_load
    handlers on reflection types. Only compiled when KRG_BUILD_EDITOR_TARGET.

    Args:
        package_ar_file: Path to output file
        fc: File context with editor type information
  """
  file_buffer = arapi.utils.FileBuffer(package_ar_file)
  editor_overrides = ""
  for include in fc.editor_overrides:
    editor_overrides += f"#include <{include}>\n"

  file_buffer.append(f"""// Smart Object Autogenerated Editor Reflection Layout
...
bool package::package_editor_enforcer()
{{
  volatile bool has_editor_types = false;
  return has_editor_types;
}}

KRG_gen__static_schedule(::kryga::gs::state::state_stage::connect,
    [](::kryga::gs::state& s)
    {{
      package::instance().register_package_extension<package::package_editor_types_builder>();
    }});

bool
package::package_editor_types_builder::build(::kryga::core::package& sp)
{{
  auto pkg = &::kryga::{fc.module_name}::package::instance();
""")

  for type_obj in fc.types:
    has_json = type_obj.json_save_handler or type_obj.json_load_handler
    if not has_json:
      continue
...
""")
    if type_obj.json_save_handler:
      file_buffer.append(f"  type_rt->json_save = {type_obj.json_save_handler};\n")
    if type_obj.json_load_handler:
      file_buffer.append(f"  type_rt->json_load = {type_obj.json_load_handler};\n")
    file_buffer.append("}")
...
bool
package::package_editor_types_builder::destroy(::kryga::core::package&)
{{
  return true;
}}
