...
#include "packages/root/model/components/animated_mesh_component.h"
...
#include "packages/root/model/assets/terrain_splatmap_material.h"
...
mesh_component__cmd_builder(reflection::type_context__render_cmd_build& ctx)
{
    ...
    auto& moc = ctx.obj->asr<root::mesh_component>();
...
{
    auto& m = ctx.obj->asr<root::mesh_component>();
...
{
    auto& lc_model = ctx.obj->asr<root::directional_light_component>();
    ...
    if (lc_model.get_selected())
    {
        ...
        if (level)
        {
            ...
            if (prev_id != lc_model.get_id() && prev_id.valid())
            {
                ...
                if (prev && prev->castable_to<root::directional_light_component>())
                {
                    auto& prev_dl = prev->asr<root::directional_light_component>();
...
{
    auto& plc_model = ctx.obj->asr<root::directional_light_component>();
...
{
    auto& lc_model = ctx.obj->asr<root::spot_light_component>();
...
{
    auto& slc_model = ctx.obj->asr<root::spot_light_component>();
...
{
    auto& lc = ctx.obj->asr<root::spot_light_component>();
...
{
    auto& lc_model = ctx.obj->asr<root::point_light_component>();
...
{
    auto& plc_model = ctx.obj->asr<root::point_light_component>();
...
{
    auto& lc = ctx.obj->asr<root::point_light_component>();
...
animated_mesh_component__cmd_builder(reflection::type_context__render_cmd_build& ctx)
{
    ...
    auto& amc = ctx.obj->asr<root::animated_mesh_component>();
...
destructible_mesh_component__cmd_builder(reflection::type_context__render_cmd_build& ctx)
{
    ...
    auto& dmc = ctx.obj->asr<root::destructible_mesh_component>();
...
float
terrain_fbm(float u, float v, const root::terrain_component& tc)
{
...
bool
build_height_field(const root::terrain_component& tc, uint32_t res, std::vector<float>& heights)
{
    heights.assign(static_cast<size_t>(res) * res, 0.0f);

    if (tc.get_source_mode() == root::terrain_source_heightmap)
    {
...
terrain_component__cmd_builder(reflection::type_context__render_cmd_build& ctx)
{
    ...
    auto& tc = ctx.obj->asr<root::terrain_component>();
