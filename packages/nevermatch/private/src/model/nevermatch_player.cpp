...
KRG_gen_class_cd_default(nevermatch_player);
...
bool
nevermatch_player::construct(this_class::construct_params& p)
{
...
void
nevermatch_player::begin_play()
{
    auto* ip = get_input()->get_input_provider();
    if (!ip)
    {
        return;
    }

    ip->register_scaled_action(AID("move_forward"), this, &nevermatch_player::on_move_forward);
    ip->register_scaled_action(AID("move_left"), this, &nevermatch_player::on_move_right);
    ip->register_scaled_action(AID("zoom"), this, &nevermatch_player::on_zoom);
    ip->register_scaled_action(AID("look_left"), this, &nevermatch_player::on_look_x);
    ip->register_scaled_action(AID("look_up"), this, &nevermatch_player::on_look_y);
    ...
    if (!get_target())
    {
        ...
        if (lvl)
        {
            set_target(lvl->find_game_object(AID("cubes_root")));
        }
    }
}

void
nevermatch_player::end_play()
{
    auto* ip = get_input()->get_input_provider();
    if (!ip)
    {
...
void
nevermatch_player::on_tick(float dt)
{
...
void
nevermatch_player::tick_snap_orbit(float dt)
{
    if (m_rotating)
    {
    ...
    get_camera()->set_look_at(m_orbit_center, m_ori * glm::vec3(0, 1, 0));
}

void
nevermatch_player::tick_mouse_orbit(float dt)
{
    auto* ip = get_input()->get_input_provider();
    ...
    if (m_cube_rotating)
    {
        ...
        if (auto* target = get_target())
        {
            ...
            target->set_rotation(root::vec3{euler.x, euler.y, euler.z});
        }
        else
        {
    ...
    set_position(root::vec3{cam_pos.x, cam_pos.y, cam_pos.z});
    get_camera()->set_look_at(m_orbit_center);
}

void
nevermatch_player::on_move_forward(float v)
{
    m_forward_input = v;
}

void
nevermatch_player::on_move_right(float v)
{
    m_right_input = v;
}

void
nevermatch_player::on_look_x(float v)
{
    m_look_x_input = v;
}

void
nevermatch_player::on_look_y(float v)
{
    m_look_y_input = v;
}

void
nevermatch_player::on_zoom(float v)
{
    m_zoom_input = v;
}
