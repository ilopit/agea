...
struct hex_coord
{
    ...
    // Equality
    constexpr bool operator==(const hex_coord& other) const { return q == other.q && r == other.r; }
    constexpr bool operator!=(const hex_coord& other) const { return !(*this == other); }

    // Arithmetic
    constexpr hex_coord operator+(const hex_coord& other) const
    {
    ...
    constexpr hex_coord operator*(int scalar) const { return hex_coord(q * scalar, r * scalar); }

    constexpr hex_coord& operator+=(const hex_coord& other)
    {
        q += other.q;
        r += other.r;
        return *this;
    ...
    static constexpr std::array<hex_coord, 6> directions()
    {
        return {{
            {1, 0},   // E
            {1, -1},  // NE
    ...
    constexpr hex_coord neighbor(int direction) const
    {
    ...
    // Get all 6 neighbors
    std::array<hex_coord, 6> neighbors() const;

    // Manhattan distance on hex grid (minimum steps to reach other)
    int distance_to(const hex_coord& other) const;

    // Convert to world space position (XZ plane, Y=0)
    // hex_size is the distance from center to corner
    glm::vec3 to_world(float hex_size) const;

    // Convert world position to hex coordinate (rounds to nearest hex)
    static hex_coord from_world(const glm::vec3& world_pos, float hex_size);

    // Get all hexes within given radius (inclusive)
    static std::vector<hex_coord> hexes_in_range(const hex_coord& center, int radius);

    // Get ring of hexes at exactly given radius
    static std::vector<hex_coord> hex_ring(const hex_coord& center, int radius);

    // Line drawing - get all hexes on line from start to end
    static std::vector<hex_coord> hex_line(const hex_coord& start, const hex_coord& end);
};

// For use in std::unordered_map/set
struct hex_coord_hash
{
    std::size_t operator()(const hex_coord& coord) const
    {
