...
void
vulkan_engine::create_material(VkPipeline pipeline,
                               render::shader_effect* effect,
                               const std::string& name)
{
    glob::render_loader::get()->create_default_material(pipeline, effect, name);
}
...
vulkan_engine::draw_new_objects(VkCommandBuffer cmd)
{
    ...
    // camera projection
    glm::mat4 projection = glm::perspective(glm::radians(70.f), w / h, 0.1f, 200.0f);
    projection[1][1] *= -1;
    ...
    render::gpu_object_data* objectSSBO = (render::gpu_object_data*)objectData;
    ...
    render::shader_descriptor_binder binder{};
    ...
    dynamicInfo.range = sizeof(render::gpu_camera_data);

    VkDescriptorSet GlobalSet;
    vk_utils::descriptor_builder::begin(device->descriptor_layout_cache(),
                                        get_current_frame().m_dynamic_descriptor_allocator)
    ...
    for (auto& r : m_rdc)
    {
        ...
        for (auto obj : r.second)
        {
            objectSSBO[objects_to_draw_idx++].model_matrix = obj->transform_matrix;
            ...
            vkCmdPushConstants(cmd, obj->material->effect->m_build_layout,
                               VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(render::mesh_push_constants),
                               &constants);

            // we can now draw

            if (!obj->mesh->has_indeces())
            {
...
{
    model::level_constructor::load_level(*glob::level::get(), "demo.plvl");

    for (auto& o : glob::level::get()->m_objects)
    {
        auto obj = o->root_component();
        obj->gather_render_objcets();
    }
}
...
vulkan_engine::compile_all_shaders()
{
    ...
    {
        for (auto& p : std::filesystem::recursive_directory_iterator(shader_compiled_dir))
        {
            if (p.is_directory())
            {
                continue;
            ...
            auto name = std::filesystem::relative(p, shader_compiled_dir);
            if (name == "nice")
            {
                continue;
            }
