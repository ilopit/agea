...
loader::load_mesh(model::mesh& mc)
{
    ...
    std::string idx_file =
        glob::resource_locator::get()->resource(category::assets, mc.m_indeces_id);

    std::vector<char> indexBuffer, vertexBuffer;

    if (!file_utils::load_file(idx_file, indexBuffer))
    {
        return false;
    }
    std::string vert_file =
        glob::resource_locator::get()->resource(category::assets, mc.m_verteces_id);
    if (!file_utils::load_file(vert_file, vertexBuffer))
    {
        return false;
    }
...
loader::load_material(model::material& d)
{
    ...
    vkCreateSampler(device->vk_device(), &samplerInfo, nullptr, &td->sampler);

    VkDescriptorImageInfo imageBufferInfo;
    imageBufferInfo.sampler = td->sampler;
