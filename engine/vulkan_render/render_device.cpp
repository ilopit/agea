...
#include "vulkan_shader_effect.h"
#include "vulkan_mesh_data.h"
...
render_device::init_commands()
{
    ...
    {
        VK_CHECK(vkCreateCommandPool(m_vk_device, &commandPoolInfo, nullptr,
                                     &m_frames[i].m_command_pool));

        // allocate the default command buffer that we will use for rendering
        VkCommandBufferAllocateInfo cmdAllocInfo =
            vk_init::command_buffer_allocate_info(m_frames[i].m_command_pool, 1);

        VK_CHECK(vkAllocateCommandBuffers(m_vk_device, &cmdAllocInfo,
...
render_device::init_sync_structures()
{
    ...
    {
        VK_CHECK(
            vkCreateFence(m_vk_device, &fenceCreateInfo, nullptr, &m_frames[i].m_render_fence));

        VK_CHECK(vkCreateSemaphore(m_vk_device, &semaphoreCreateInfo, nullptr,
                                   &m_frames[i].m_present_semaphore));
        VK_CHECK(vkCreateSemaphore(m_vk_device, &semaphoreCreateInfo, nullptr,
                                   &m_frames[i].m_render_semaphore));
    }
...
{
    m_descriptor_allocator = new vk_utils::descriptor_allocator{};
    m_descriptor_allocator->init(m_vk_device);

    m_descriptor_layout_cache = new vk_utils::descriptor_layout_cache{};
    ...
    const size_t sceneParamBufferSize =
        FRAMES_IN_FLYIGNT * pad_uniform_buffer_size(sizeof(gpu_scene_data));

    for (int i = 0; i < FRAMES_IN_FLYIGNT; i++)
    {
        m_frames[i].m_dynamic_descriptor_allocator = new vk_utils::descriptor_allocator{};
        m_frames[i].m_dynamic_descriptor_allocator->init(m_vk_device);

        const int MAX_OBJECTS = 10000;
        m_frames[i].m_object_buffer =
        ...
        m_frames[i].m_dynamic_data_buffer = create_buffer(
            10000000, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VMA_MEMORY_USAGE_CPU_TO_GPU);
    }
    return true;
}
...
{
    auto mesh_module = glob::render_loader::get()->load_shader("tri_mesh_ssbo.vert.spv");
    if (!mesh_module)
    {
    ...
    auto texture_module = glob::render_loader::get()->load_shader("textured_lit.frag.spv");
    if (!texture_module)
    {
        std::cout << "Error when building the colored mesh shader" << std::endl;
        return false;
    ...
    shader_effect* textured_effect = new shader_effect();
    ...
    glob::engine::get()->create_material(texture_pipeline, textured_effect, "simple_texture");
...
render_device::wait_for_fences()
{
    for (auto& frame : m_frames)
    {
        vkWaitForFences(m_vk_device, 1, &frame.m_render_fence, true, 1000000000);
    }
}
