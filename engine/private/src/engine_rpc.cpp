...
void
rpc_transform_get(const Json::Value& params, Json::Value& result, std::string& err)
{
    auto& eng = glob::glob_state().getr_engine();
    if (!params.isObject() || !params.isMember("id"))
    {
...
void
rpc_transform_set(const Json::Value& params, Json::Value& result, std::string& err)
{
    auto& eng = glob::glob_state().getr_engine();
    if (!params.isObject() || !params.isMember("id"))
    {
    ...
    bool done = eng.wait_main_action(
        [&]()
        {
            ...
            {
                goc->set_scale({scl_val[0].asFloat(), scl_val[1].asFloat(), scl_val[2].asFloat()});
            }
        });
    if (!done)
    {
