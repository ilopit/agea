...
bool
package::load_package(const std::string& path, package& p, cache_set_ref global_cs)
{
    ALOG_INFO("Loading package {0}", path);

    p.m_global_cs = global_cs;

    p.m_path = path;
    p.m_id = std::filesystem::path(path).filename().generic_string();
    p.m_occ = std::make_unique<object_constructor_context>(p.m_global_cs, p.m_local_cs.get_ref(),
                                                           &p.m_objects);
    p.m_occ->m_full_path = p.m_path;
...
bool
package::save_package(const std::string& path, const package& p)
{
    ALOG_INFO("Loading package {0}", path);
    ...
    {
        auto obj_path = p.get_resource_path(get_name(o->get_architype_id()));
        ...
        if (!object_constructor::class_object_save(*o, obj_path))
        {
            ALOG_LAZY_ERROR;
            return false;
        }
...
{
    auto module_path = p.m_path + "/" + get_name(id);
    ...
    for (auto& resource : std::filesystem::directory_iterator(module_path))
    {
        ...
        auto obj =
            model::object_constructor::class_object_load(package_path.generic_string(), *p.m_occ);
