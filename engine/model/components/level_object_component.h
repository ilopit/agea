#pragma once

#include "model/components/component.h"

#include "agea_minimal.h"

#include "wrappers/glm_wrapper.h"

namespace agea
{
struct render_data;

namespace model
{
const extern glm::vec3 DEF_FORWARD;
const extern glm::vec3 DEF_UP;
const extern glm::vec3 DEF_RIGHT;

class level_object_component : public component
{
public:
    AGEA_gen_class_meta(level_object_component, component);
    AGEA_gen_construct_params
    {
        glm::vec3 position = glm::vec3(0.f, 0.f, 0.f);
        glm::vec3 scale = glm::vec3(1.f, 1.f, 1.f);
        glm::vec3 rotation = glm::vec3(0.f, 0.f, 0.f);
    };
    AGEA_gen_meta_api;

    bool
    construct(construct_params& c)
    {
        AGEA_return_nok(base_class::construct(c));

        m_position = c.position;
        m_rotation = c.rotation;
        m_scale = c.scale;

        return true;
    }

    bool clone(this_class& src);

    glm::vec3
    position() const
    {
        return m_position;
    }

    void
    set_position(glm::vec3 p)
    {
        m_position = p;
    }

    glm::quat
    rotation() const
    {
        return m_rotation;
    }

    void
    set_rotation(glm::quat q)
    {
        // m_rotation = q;
    }

    glm::vec3
    scale() const
    {
        return m_scale;
    }

    void
    set_scale(glm::vec3 s)
    {
        m_scale = s;
    }

    glm::vec3
    forward_vector() const
    {
        return glm::rotate(glm::quat(m_rotation), DEF_FORWARD);
    }

    glm::vec3
    up_vector() const
    {
        return glm::rotate(glm::quat(m_rotation), DEF_UP);
    }

    glm::vec3
    right_vector() const
    {
        return glm::rotate(glm::quat(m_rotation), DEF_RIGHT);
    }

    void
    move(const glm::vec3& delta)
    {
        m_position += delta;
    }

    void
    rotate(float delta_angle, const glm::vec3& axis)
    {
        m_rotation = glm::rotate(m_rotation, delta_angle, axis);
    }

    void
    roll(float delta_angle)
    {
        rotate(delta_angle, DEF_FORWARD);
    }

    void
    yaw(float delta_angle)
    {
        rotate(delta_angle, DEF_UP);
    }

    void
    pitch(float delta_angle)
    {
        rotate(delta_angle, DEF_RIGHT);
    }

    void gather_render_objcets();

    virtual void
    set_parent(component* c)
    {
        base_class::set_parent(c);

        if (c->castable_to<level_object_component>())
        {
            m_render_root = (level_object_component*)c;
        }
    }

    virtual void
    attach(component* c)
    {
        base_class::attach(c);

        AGEA_cond(c->castable_to<level_object_component>(), "Not renderable!");
        m_render_components.push_back((level_object_component*)c);
    }

    virtual bool prepare_for_rendering();

    template <typename enter_action, typename exit_action>
    void
    run_over_components(const enter_action& enter, const exit_action& exit)
    {
        enter(*this);

        for (auto rc : m_render_components)
        {
            rc->run_over_components(enter, exit);
        }
        exit(*this);
    }

    void update_matrix();
    glm::mat4 transofrm_matrix();

    struct render_data* m_render_object = nullptr;

    bool deserialize_finalize(json_conteiner& c);

protected:
    glm::vec3 m_position;
    glm::vec3 m_rotation;
    glm::vec3 m_scale;

    level_object_component* m_render_root = nullptr;
    std::vector<level_object_component*> m_render_components;

    std::vector<std::shared_ptr<component>> m_components;
};

}  // namespace model
}  // namespace agea
