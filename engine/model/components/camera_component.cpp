...
{
    auto owner_obj = (level_object*)owner();
    m_view =
        glm::lookAt(owner_obj->position(), owner_obj->position() + forward_vector(), up_vector());
}
...
{
    auto owner_obj = (level_object*)owner();
    m_model = glm::translate(owner_obj->position()) * glm::toMat4(owner_obj->rotation()) * m_scale;
}
