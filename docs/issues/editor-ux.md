...
Audit of the ImGui-based editor experience. Covers workflow gaps, interaction model, and visual polish.

---

## Workflow Gaps

### 1. No undo/redo [major]
...
### 2. No object deletion from editor [major]
...
**Fix:** Bind Delete key, confirm on multi-component objects, push through undo system (item 1).

### 3. No copy/paste/duplicate objects [medium]

No Ctrl+C/V/D bindings. Duplication only via RPC (`scene.duplicate`). No clipboard for cross-level object transfer.

### 4. No multi-selection [medium]
...
### 5. No scene search or filter [medium]
...
### 9. No keyboard shortcut discovery [low-medium]
...
### 10. CollapsingHeader state not persisted [low]

ImGui only saves window-level state in `imgui.ini`. Section expanded/collapsed state within panels (render config sections) resets every session.

**Options:** `ImGui::GetStateStorage()` with manual save/load to rtcache, or store bools in config files.

### 11. Perf counter update stutter [low]
...
### 12. Selection outline requires render_scale [low, architectural]
...
### 13. Hardcoded layout values in gizmo editor [low]
